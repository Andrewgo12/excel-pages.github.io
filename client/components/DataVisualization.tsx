...
export const DataVisualization: React.FC<DataVisualizationProps> = ({
  ...
  const overviewData = useMemo(() => {
    return {
      ...
      completeness: stats.map(stat => ({
        column: stat.column.length > 15 ? stat.column.substring(0, 12) + '...' : stat.column,
        completitud: Math.round((stat.count / (stat.count + stat.nullCount)) * 100),
        valores: stat.count,
        nulos: stat.nullCount
      })).slice(0, 10),

      uniqueness: stats.map(stat => ({
        column: stat.column.length > 15 ? stat.column.substring(0, 12) + '...' : stat.column,
  ...
  const customChartData = useMemo(() => {
    ...
    if (xColumn.type === 'text') {
      ...
      return Object.values(grouped).map((group: any) => ({
        ...
        value: yColumn.type === 'number' ? Math.round(group.total / group.count * 100) / 100 : group.count,
        total: group.total,
        count: group.count
      })).slice(0, 15);
    }
    ...
    })).filter(item => item.x !== null && item.y !== null).slice(0, 50);
  ...
  const dateColumns = columns.filter(col => col.type === 'date' && selectedColumns.includes(col.key));
      ...
      case 'overview':
        return (
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Distribución de Tipos de Datos</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Completitud de Datos por Columna</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <BarChart data={overviewData.completeness}>
                    ...
                    <Tooltip 
                      formatter={(value, name) => [
                        ...
                        name === 'completitud' ? 'Completitud' : name === 'valores' ? 'Valores' : 'Nulos'
                      ]}
                    />
                    <Legend />
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Unicidad de Valores por Columna</CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={overviewData.uniqueness}>
                    ...
                    <Tooltip 
                      formatter={(value, name) => [
                        ...
                        name === 'unicidad' ? 'Unicidad' : name === 'unicos' ? 'Únicos' : 'Total'
                      ]}
                    />
                    <Legend />
        ...
        {selectedChart === 'custom' && (
          <>
            <Select value={xAxis} onValueChange={setXAxis}>
              ...
              <SelectContent>
                {[...textColumns, ...numericColumns, ...dateColumns].map(col => (
