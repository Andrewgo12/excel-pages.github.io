...
import { Wand2, Trash2, Copy, Edit, Sparkles, Eye, Play, X, Plus } from 'lucide-react';
...
export const BulkOperations: React.FC<BulkOperationsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'transform' | 'bulk' | 'clean'>('transform');
  const [transformations, setTransformations] = useState<TransformationRule[]>([]);
  const [newTransformation, setNewTransformation] = useState<Partial<TransformationRule>>({});
  const [previewDialogOpen, setPreviewDialogOpen] = useState(false);
  const [previewRule, setPreviewRule] = useState<TransformationRule | null>(null);
  
  // Bulk operations state
  const [bulkOperation, setBulkOperation] = useState<'delete' | 'update' | 'duplicate'>('delete');
  ...
  const [fillValues, setFillValues] = useState<{ column: string; value: any }[]>([]);
  ...
  const addTransformation = () => {
    if (!newTransformation.name || !newTransformation.type || !newTransformation.targetColumn) return;
    ...
    try {
      ...
      const condition = (row: Record<string, any>) => {
        return Object.values(row).some(value => 
          String(value || '').toLowerCase().includes(bulkFilter.toLowerCase())
        );
      };
  ...
  const applyDataCleaning = () => {
    ...
    const summary = [
      result.cleaned.duplicates > 0 ? `${result.cleaned.duplicates} duplicados eliminados` : '',
      result.cleaned.emptyRows > 0 ? `${result.cleaned.emptyRows} filas vacías eliminadas` : '',
      result.cleaned.trimmed > 0 ? `${result.cleaned.trimmed} espacios eliminados` : '',
      result.cleaned.filled > 0 ? `${result.cleaned.filled} valores rellenados` : ''
    ].filter(Boolean).join(', ');
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Select value={newTransformation.type || ''} onValueChange={(value: any) => setNewTransformation({...newTransformation, type: value})}>
                <SelectTrigger>
                  <SelectValue placeholder="Seleccionar tipo" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="replace">Reemplazar/Transformar</SelectItem>
            ...
            <div>
              ...
              <Select 
                ...
                onValueChange={(value) => setNewTransformation({...newTransformation, sourceColumns: [value]})}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setNewTransformation({...newTransformation, targetColumn: e.target.value})}
                ...
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewTransformation({
                      ...newTransformation, 
                      parameters: {...newTransformation.parameters, operation: value}
                    })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                  ...
                  <>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewTransformation({
                          ...newTransformation,
                          parameters: {...newTransformation.parameters, search: e.target.value}
                        })}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewTransformation({
                          ...newTransformation,
                          parameters: {...newTransformation.parameters, replace: e.target.value}
                        })}
                ...
                {(newTransformation.parameters?.operation === 'multiply' || 
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewTransformation({
                        ...newTransformation,
                        parameters: {...newTransformation.parameters, value: Number(e.target.value)}
                      })}
                    />
                  </div>
                )}
              ...
              <>
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewTransformation({
                      ...newTransformation, 
                      sourceColumns: [newTransformation.sourceColumns?.[0] || '', value]
                    })}
                  >
                    <SelectTrigger>
            ...
            <div className="flex items-end">
              <Button 
                ...
                disabled={!newTransformation.name || !newTransformation.type || !newTransformation.targetColumn}
                className="mr-2"
              >
              ...
              {newTransformation.name && newTransformation.type && newTransformation.targetColumn && (
                <Button 
                  variant="outline" 
                  onClick={() => {
                    const rule: TransformationRule = {
        ...
        <CardContent>
          <div className="space-y-4">
                  ...
                  {availableColumns.slice(0, 3).map(col => (
                    <div key={col.key} className="flex gap-2 items-center">
                      ...
                      <Input
                        ...
                        onChange={(e) => setBulkUpdates({...bulkUpdates, [col.key]: e.target.value})}
            ...
            <Button 
              ...
              className="w-full"
            >
              {bulkOperation === 'delete' && <Trash2 className="h-4 w-4 mr-2" />}
              {bulkOperation === 'update' && <Edit className="h-4 w-4 mr-2" />}
              {bulkOperation === 'duplicate' && <Copy className="h-4 w-4 mr-2" />}
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setCleaningOptions({...cleaningOptions, removeDuplicates: !!checked})}
                />
                <Label htmlFor="removeDuplicates">Eliminar filas duplicadas</Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setCleaningOptions({...cleaningOptions, removeEmptyRows: !!checked})}
                />
                <Label htmlFor="removeEmptyRows">Eliminar filas completamente vacías</Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setCleaningOptions({...cleaningOptions, trimWhitespace: !!checked})}
                />
                <Label htmlFor="trimWhitespace">Eliminar espacios en blanco extra</Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setCleaningOptions({...cleaningOptions, fillEmptyValues: !!checked})}
                />
                  ...
                  <div className="space-y-2">
                    {availableColumns.slice(0, 5).map(col => (
                      <div key={col.key} className="flex gap-2 items-center">
                        <Label className="w-32">{col.label}:</Label>
                        <Input
                          value={fillValues.find(f => f.column === col.key)?.value || ''}
                          onChange={(e) => {
                            const newFillValues = fillValues.filter(f => f.column !== col.key);
                            if (e.target.value) {
                              newFillValues.push({ column: col.key, value: e.target.value });
                            }
