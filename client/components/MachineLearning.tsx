...
export function MachineLearning({
  ...
  const numericColumns = useMemo(
    () =>
      getNumericColumns(data.columns).filter((col) =>
  ...
  const categoricalColumns = useMemo(
    () =>
      getCategoricalColumns(data.columns).filter((col) =>
        selectedColumns.includes(col.key),
      ),
    [data.columns, selectedColumns],
  );
  ...
  const availableFeatures = useMemo(() => {
    return data.columns.filter(
      (col) => selectedColumns.includes(col.key) && col.key !== targetColumn,
    );
          ...
          <TabsContent value="setup" className="space-y-responsive">
            ...
            <div className="space-y-responsive">
              ...
              <div>
                ...
                <Select
                  value={modelType}
                  onValueChange={(value: "linear_regression" | "naive_bayes") =>
                    setModelType(value)
                  }
                >
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handleFeatureToggle(col.key, !!checked)
                          }
                        />
                        <Label
                          htmlFor={col.key}
                          className="text-responsive-sm flex-1 cursor-pointer"
                        >
                          {col.label}
                        </Label>
                        <Badge
                    ...
                    <div className="grid grid-cols-2 gap-responsive text-responsive-sm">
                      <div>
                        ...
                        <div className="font-medium">
                          {trainedModel.config.targetColumn}
                        </div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Features:</span>
                        <div className="font-medium">
                          {trainedModel.config.featureColumns.length}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="font-medium">
                          {Math.round(
                            filteredData.length *
                              trainedModel.config.trainTestSplit,
                          )}
                        </div>
                      </div>
                      <div>
                  ...
                  <CardContent className="p-responsive-sm">
                    {trainedModel.config.modelType === "linear_regression" ? (
                      <div className="space-y-responsive-sm">
                        {(() => {
                          const metrics =
                            trainedModel.modelMetrics as LinearRegressionResult;
                          return (
                            <>
                              <div className="flex items-center justify-between">
                      ...
                      <div className="space-y-responsive-sm">
                        {(() => {
                          const metrics =
                            trainedModel.modelMetrics as ClassificationResult;
                          return (
                            <>
                              <div className="flex items-center justify-between">
                                <span className="text-responsive-sm">
                              ...
                              <div className="mt-responsive">
                                <div className="text-responsive-sm font-medium mb-2">
                                  Métricas por Clase
                                </div>
                                <div className="space-y-1">
                                  {Object.keys(metrics.precision).map(
                                    (className) => (
                                      <div
                                        key={className}
                                        className="text-responsive-xs"
                  ...
                  <div className="space-y-responsive-sm">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-responsive-sm">
                                <span className="text-responsive-sm">
                                  Predicho:{" "}
                                  <span className="font-mono">
                                    {typeof prediction.predictedValue ===
                                    "number"
                                ...
                                {prediction.residual !== undefined && (
                                  <Badge
                                    variant={
                                      Math.abs(prediction.residual) < 1
                                        ? "default"
                                        : "secondary"
                                    }
                                    className="text-responsive-xs"
                    ...
                    {trainedModel.predictions.length > 50 && (
                      <div className="text-center text-responsive-sm text-muted-foreground py-responsive">
                        ... y {trainedModel.predictions.length - 50}{" "}
                        predicciones más
                      </div>
                    )}
                  </div>
