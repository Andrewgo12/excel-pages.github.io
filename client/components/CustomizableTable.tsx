...
interface CustomizableTableProps {
  ...
  onCellClick?: (value: any, column: ExcelColumn, row: Record<string, any>) => void;
  renderCell?: (value: any, column: ExcelColumn, row: Record<string, any>) => React.ReactNode;
...
export const CustomizableTable: React.FC<CustomizableTableProps> = ({
  ...
  const getCellAlignment = (column: ExcelColumn) => {
    const columnCustomization = getColumnCustomization(customization, column.key);
    return columnCustomization.alignment || customization.defaultAlignment;
  };

  const getCellStyle = (column: ExcelColumn, isHeader = false) => {
    const columnCustomization = getColumnCustomization(customization, column.key);
    const alignment = columnCustomization.alignment || (isHeader ? customization.headerAlignment : customization.defaultAlignment);
    const colorScheme = isHeader ? customization.headerColorScheme : customization.colorScheme;
    ...
    return {
      ...
      lineHeight: columnCustomization.fontSettings?.lineHeight || font.lineHeight,
      ...
      width: columnCustomization.width ? `${columnCustomization.width}px` : undefined,
      minWidth: columnCustomization.minWidth ? `${columnCustomization.minWidth}px` : undefined,
      maxWidth: columnCustomization.maxWidth ? `${columnCustomization.maxWidth}px` : undefined,
    };
  };

  const getRowStyle = (rowIndex: number) => {
    const isStriped = customization.showStriping && 
                     customization.alternateRowColors && 
                     (rowIndex % customization.stripingInterval === 0);
    
    return {
      backgroundColor: isStriped ? customization.colorScheme.striped : customization.colorScheme.background,
  ...
  const formatCellValue = (value: any, column: ExcelColumn) => {
    ...
    switch (columnCustomization.formatter) {
      ...
      case 'custom':
        if (columnCustomization.customFormatter) {
          try {
            ...
            return columnCustomization.customFormatter.replace(/\{value\}/g, String(value));
          } catch {
            return String(value);
          }
        }
      ...
      default:
        ...
        switch (column.type) {
          case 'number':
            return isNaN(Number(value)) ? String(value) : Number(value).toLocaleString('es-ES');
          case 'boolean':
            return value ? 'Sí' : 'No';
          case 'date':
            try {
  ...
  const tableClass = cn(
    ...
    customization.stickyHeader && "[&_thead_th]:sticky [&_thead_th]:top-0 [&_thead_th]:z-10"
  );
  ...
  return (
    ...
    >
            ...
            {visibleColumns.map((column) => (
              <TableHead
                ...
                className={cn(
                  ...
                  onSort && "cursor-pointer hover:bg-opacity-80 transition-colors"
                )}
        ...
        <TableBody>
          {data.map((row, rowIndex) => (
            <TableRow
              ...
              className={cn(
                customization.showHoverEffects && "hover:opacity-80 transition-all duration-200",
                onRowClick && "cursor-pointer"
              )}
              onClick={() => onRowClick?.(row, rowIndex)}
              onMouseEnter={(e) => {
                if (customization.showHoverEffects) {
                  e.currentTarget.style.backgroundColor = customization.colorScheme.hover;
                }
              }}
              onMouseLeave={(e) => {
                if (customization.showHoverEffects) {
            ...
            >
              {visibleColumns.map((column) => {
                ...
                return (
                  ...
                  >
                    {renderCell ? renderCell(value, column, row) : (
      ...
      {customization.virtualization && data.length > customization.virtualizationThreshold && (
