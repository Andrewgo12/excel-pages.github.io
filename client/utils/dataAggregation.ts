...
export const aggregateData = (
  ...
  const results: AggregatedResult[] = Object.entries(groups).map(
    ([groupKey, groupRows]) => {
...
export const createPivotTable = (
  ...
  const rowValues = [
    ...new Set(data.map((row) => String(row[rowColumn] || "Sin valor"))),
  ];
  const colValues = [
    ...new Set(data.map((row) => String(row[columnColumn] || "Sin valor"))),
  ];
  ...
  rowValues.forEach((rowVal) => {
    ...
    colValues.forEach((colVal) => {
      ...
      const matchingRows = data.filter(
        (row) =>
          String(row[rowColumn] || "Sin valor") === rowVal &&
