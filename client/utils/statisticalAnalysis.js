export const calculateColumnStats = (data, column) => {
  ...
  if (column.type === "text") {
    ...
    if (textValues.length > 0) {
      ...
      stats.avgLength =
        Math.round(
          (lengths.reduce((acc, len) => acc + len, 0) / lengths.length) * 100,
        ) / 100;
  ...
  if (column.type === "date") {
    ...
    if (dateValues.length > 0) {
      ...
      stats.dateRange = Math.ceil(
        (stats.maxDate.getTime() - stats.minDate.getTime()) /
          (1000 * 60 * 60 * 24),
      );
    }
  }
...
export const calculateDatasetStats = (data, columns) => {
  ...
  const completenessScore =
    totalCells > 0 ? Math.round((filledCells / totalCells) * 100 * 10) / 10 : 0;
