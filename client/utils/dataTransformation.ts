...
export interface TransformationConfig {
  handleMissingValues: 'remove' | 'fill_mean' | 'fill_median' | 'fill_mode' | 'fill_custom' | 'interpolate';
  customFillValue?: any;
...
export interface DerivedColumnRule {
  ...
  operation: 'sum' | 'average' | 'concat' | 'difference' | 'ratio' | 'conditional' | 'formula';
...
export const detectMissingValues = (
  ...
  const missingReport: Record<string, { count: number; percentage: number; indices: number[] }> = {};
  ...
  columns.forEach(column => {
    const missing = data
      ...
      .filter(item => 
        ...
        (typeof item.value === 'string' && item.value.toLowerCase() === 'null') ||
        (typeof item.value === 'string' && item.value.toLowerCase() === 'n/a') ||
...
export const fillMissingValues = (
  ...
  const columnData = data.map(row => row[column]).filter(val => 
    val !== null && val !== undefined && val !== '' && val !== 'null' && val !== 'n/a' && val !== 'na'
  );
  ...
  switch (strategy) {
    case 'mean':
      const numbers = columnData.filter(val => !isNaN(Number(val))).map(Number);
      fillValue = numbers.length > 0 ? numbers.reduce((a, b) => a + b, 0) / numbers.length : 0;
      break;
    
    case 'median':
      const sortedNumbers = columnData.filter(val => !isNaN(Number(val))).map(Number).sort((a, b) => a - b);
      fillValue = sortedNumbers.length > 0 ? 
        sortedNumbers[Math.floor(sortedNumbers.length / 2)] : 0;
      break;
    ...
    case 'interpolate':
      ...
      return data.map((row, index) => {
        if (row[column] === null || row[column] === undefined || row[column] === '') {
          const prevIndex = data.slice(0, index).findLastIndex(r => 
            r[column] !== null && r[column] !== undefined && r[column] !== ''
          );
          const nextIndex = data.slice(index + 1).findIndex(r => 
            r[column] !== null && r[column] !== undefined && r[column] !== ''
          ) + index + 1;
          ...
          if (prevIndex >= 0 && nextIndex < data.length) {
            ...
            if (!isNaN(prevValue) && !isNaN(nextValue)) {
              const interpolated = prevValue + ((nextValue - prevValue) * (index - prevIndex) / (nextIndex - prevIndex));
  ...
  return data.map(row => {
    if (row[column] === null || row[column] === undefined || row[column] === '' ||
        (typeof row[column] === 'string' && ['null', 'n/a', 'na'].includes(row[column].toLowerCase()))) {
...
export const detectDuplicates = (
  ...
  const compareColumns = columns || Object.keys(data[0] || {}).filter(key => key !== '_id');
  ...
  duplicateGroups.forEach(group => {
    ...
    if (keepStrategy === 'last') {
      ...
      keepIndex = group.reduce((best, current) => {
        const bestNonNull = Object.values(data[best]).filter(val => 
          val !== null && val !== undefined && val !== ''
        ).length;
        const currentNonNull = Object.values(data[current]).filter(val => 
          val !== null && val !== undefined && val !== ''
        ).length;
...
export const handleOutliers = (
  ...
  return data.map((row, index) => {
    if (outlierIndices.includes(index)) {
      ...
      if (method === 'cap') {
        const cappedValue = value < lowerBound ? lowerBound : 
...
export const createDerivedColumn = (
  ...
  return data.map(row => {
    ...
    switch (rule.operation) {
      case 'sum':
        derivedValue = rule.sourceColumns.reduce((sum, col) => 
          sum + (Number(row[col]) || 0), 0);
        break;
      ...
      case 'average':
        ...
        derivedValue = validValues.length > 0 ? 
      ...
      case 'difference':
        if (rule.sourceColumns.length >= 2) {
          derivedValue = (Number(row[rule.sourceColumns[0]]) || 0) - 
      ...
      case 'ratio':
        if (rule.sourceColumns.length >= 2) {
          ...
          derivedValue = (Number(row[rule.sourceColumns[0]]) || 0) / denominator;
        }
        break;
      ...
      case 'conditional':
        ...
        derivedValue = row[rule.sourceColumns[0]] === condition ? trueValue : falseValue;
        break;
      ...
      case 'formula':
        ...
        try {
          ...
          rule.sourceColumns.forEach(col => {
            formula = formula.replace(new RegExp(`{${col}}`, 'g'), Number(row[col]) || 0);
          });
...
export const cleanData = (
  ...
  if (config.handleMissingValues !== 'remove') {
    columns.forEach(column => {
      ...
      const changes = cleanedData.filter((row, i) => 
        originalData[i][column.key] !== row[column.key]
      ).length;
  ...
  if (config.handleOutliers !== 'ignore') {
    ...
    numericColumns.forEach(column => {
      ...
      if (outliers.length > 0) {
        const beforeCount = cleanedData.length;
        cleanedData = handleOutliers(cleanedData, column.key, outliers, config.handleOutliers as any);
  ...
  if (config.normalizeText) {
    const textColumns = columns.filter(col => col.type === 'text').map(col => col.key);
  ...
  if (config.validateDataTypes) {
    const { correctedData, issues: typeIssues } = validateAndCorrectDataTypes(cleanedData, columns);
