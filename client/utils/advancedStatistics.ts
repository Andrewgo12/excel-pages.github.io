...
export const classifyCorrelationStrength = (
  correlation: number,
): {
  strength: "very_weak" | "weak" | "moderate" | "strong" | "very_strong";
  direction: "positive" | "negative";
  ...
  for (let i = 0; i < numericColumns.length; i++) {
    for (let j = i + 1; j < numericColumns.length; j++) {
      ...
      if (values1.length > 2 && values2.length > 2) {
        ...
        const significance =
          1 - Math.abs(tStat) / (Math.abs(tStat) + Math.sqrt(n - 2));
...
export const testNormality = (
  values: number[],
): { isNormal: boolean; statistic: number } => {
  ...
  const skewness =
    values.reduce((sum, x) => sum + Math.pow((x - mean) / stdDev, 3), 0) / n;
  const kurtosis =
    values.reduce((sum, x) => sum + Math.pow((x - mean) / stdDev, 4), 0) / n -
    3;
