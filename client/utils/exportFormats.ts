...
export const exportToExcel = (
  ...
  try {
    ...
    const metadataSheet = XLSX.utils.json_to_sheet([
      {
        Propiedad: "Fecha de exportación",
      ...
      {
        Propiedad: "Columnas exportadas",
        Valor: filteredColumns.map((c) => c.label).join(", "),
      },
    ]);
...
export const exportToPDF = (
  ...
  try {
    ...
    // Prepare table data
    const headers = filteredColumns.map((col) => col.label);
    const tableData = data.slice(0, 1000).map(
      (
        row, // Limit to 1000 rows for PDF
      ) =>
