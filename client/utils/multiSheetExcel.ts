...
export const loadCompleteExcelFile = async (file: File): Promise<{ data: ExcelData; analysis: MultiSheetAnalysis }> => {
        ...
        workbook.SheetNames.forEach((sheetName) => {
          ...
          if (mainRegion) {
            ...
            const rows = jsonData
              ...
              .map((row: any[], index) => {
                const rowData: Record<string, any> = { _id: `${sheetName}_${index}` };
...
const analyzeSheetStructure = (jsonData: any[][], sheetName: string): SheetAnalysis => {
...
const detectTableRegions = (jsonData: any[][]): TableRegion[] => {
  ...
  for (let startRow = 0; startRow < Math.min(jsonData.length, 10); startRow++) {
    ...
    const nonEmptyHeaders = row.filter(cell => 
    ...
    for (let checkRow = startRow + 1; checkRow < jsonData.length; checkRow++) {
      ...
      const nonEmptyData = dataRow.filter(cell => 
        cell !== undefined && cell !== null && cell !== ""
      );
    ...
    if (hasData && endRow > startRow) {
      const startCol = 0;
      const endCol = Math.max(...jsonData.slice(startRow, endRow + 1).map(row => row.length)) - 1;
      ...
      regions.push({
        ...
        startCol,
        endCol,
        columnHeaders: row.slice(startCol, endCol + 1).map(h => String(h || `Col${startCol + row.indexOf(h)}`)),
        rowCount: endRow - startRow,
        confidence: calculateTableConfidence(jsonData, startRow, endRow, startCol, endCol)
      });
...
const calculateTableConfidence = (
  ...
  if (headers) {
    const textHeaders = headers.slice(startCol, endCol + 1).filter(h => 
      typeof h === 'string' && h.length > 0 && isNaN(Number(h))
    ).length;
  ...
  for (let row = startRow + 1; row <= endRow; row++) {
    ...
    if (dataRow) {
      const nonEmptyData = dataRow.slice(startCol, endCol + 1).filter(cell => 
        cell !== undefined && cell !== null && cell !== ""
      ).length;
      
      if (nonEmptyData >= expectedColumns * 0.5) { // At least 50% filled
        consistentRows++;
      }
    }
  }
...
// Analyze relationships between sheets
const analyzeSheetRelationships = (sheetsData: Record<string, SheetData>): SheetRelationship[] => {
      ...
      sourceData.columns.forEach(sourceCol => {
        targetData.columns.forEach(targetCol => {
          const relationship = analyzeColumnRelationship(
            sourceData.rows, sourceCol.key,
            targetData.rows, targetCol.key
          );
          
          if (relationship.matchPercentage > 0.1) { // At least 10% match
            relationships.push({
              sourceSheet,
              targetSheet,
...
const analyzeColumnRelationship = (
  ...
  const sourceValues = sourceRows.map(row => String(row[sourceColumn] || "")).filter(v => v !== "");
  const targetValues = targetRows.map(row => String(row[targetColumn] || "")).filter(v => v !== "");
  ...
  const matchPercentage = (matchCount / Math.max(sourceSet.size, targetSet.size)) * 100;
  
  // Determine relationship type
  let relationship: 'one_to_one' | 'one_to_many' | 'many_to_many' = 'one_to_one';
...
const determineRecommendedSheet = (analyses: SheetAnalysis[]): string => {
  ...
  const scored = analyses.map(analysis => {
    ...
    if (analysis.name.toLowerCase().includes('data') || 
...
const calculateComplexity = (
  ...
  const totalTables = analyses.reduce((sum, a) => sum + a.tableRegions.length, 0);
  const relationshipCount = relationships.filter(r => r.confidence > 0.3).length;
...
export const optimizeSheetForDisplay = (sheetData: SheetData, maxRows: number = 1000): SheetData => {
  ...
  const sampledRows = sheetData.rows.filter((_, index) => index % sampleStep === 0);
...
export const preloadSheet = async (sheetData: SheetData): Promise<SheetData> => {
