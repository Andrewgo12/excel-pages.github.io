  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const mainSheetData = sheetsData[activeSheet] || {
          columns: [],
          rows: [],
        };
...
const analyzeMultiSheetStructure = (sheetsData, sheetNames) => {
      ...
      const commonColumns = sheet1.columns.filter((col1) =>
        sheet2.columns.some(
          (col2) =>
            ...
            (col1.label.toLowerCase().includes("id") &&
              col2.label.toLowerCase().includes("id")),
        ),
      );
  ...
  const totalRows = sheetAnalyses.reduce(
  ...
  const recommendedStartSheet =
    nonEmptySheets.length > 0
