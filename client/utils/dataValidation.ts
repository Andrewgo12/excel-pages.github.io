...
export const validateUnique = (
  ...
  const duplicates = data.filter(
    (row, index) => index !== currentIndex && row[column] === currentValue,
  );
...
export const validateData = (
      ...
      switch (rule.type) {
        ...
        case "pattern":
          isValid = validatePattern(
            ...
            rule.parameters.flags,
          );
          message = isValid
            ? ""
            : `Valor ${value} no coincide con el patrón requerido`;
          suggestion =
            rule.parameters.suggestion || "Revisa el formato requerido";
          break;
        ...
        case "enum":
          ...
          message = isValid
            ? ""
            : `Valor ${value} no está en la lista de valores permitidos`;
        ...
        case "date_range":
          isValid = validateDateRange(
            value,
            rule.parameters.minDate,
            rule.parameters.maxDate,
          );
          if (!isValid) {
  ...
  const validRows =
    data.length -
    new Set(issues.filter((i) => i.severity === "error").map((i) => i.rowIndex))
      .size;
  ...
  const qualityScore =
    data.length > 0
      ? Math.max(
          0,
          100 -
            (errorCount / data.length) * 100 -
            (warningCount / data.length) * 30,
        )
      : 100;
...
export const generateAutoValidationRules = (
  ...
  columns.forEach((column) => {
    ...
    if (
      column.type === "text" &&
      values.some((val) => String(val).includes("@"))
    ) {
    ...
    if (
      (column.type === "text" &&
        column.key.toLowerCase().includes("telefono")) ||
      ...
      if (numericValues.length > 0) {
        ...
        if (max - min > 0 && numericValues.every((val) => val >= 0)) {
          rules.push({
            ...
            severity: "warning",
            description:
              "Auto-detectado: rango numérico basado en datos existentes",
          });
        }
      }
    }
