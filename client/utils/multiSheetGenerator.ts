  ...
  const generateRandomSalesData = (count: number) => {
    return Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        categoria_producto: ["Electrónicos", "Informática", "Accesorios"][
        ...
        region_venta: ["Norte", "Sur", "Este", "Oeste", "Centro"][
          Math.floor(Math.random() * 5)
        ],
        canal: ["Online", "Tienda", "Teléfono", "Partner"][
        ...
        campaña: ["Black Friday", "Navidad", "Verano", "Sin campaña"][
          Math.floor(Math.random() * 4)
        ],
  ...
  const generateRandomProjectData = (count: number) => {
    return Array.from({ length: count }, (_, index) => {
      ...
      const fechaFinEstimada = new Date(
        fechaInicio.getTime() + Math.random() * 365 * 24 * 60 * 60 * 1000,
      );
      ...
      return {
        ...
        fecha_fin_real:
          progreso === 100 ? fechaFinEstimada.toLocaleDateString("es-ES") : "",
        estado_proyecto: ["En Progreso", "Completado", "En Pausa", "Cancelado"][
          Math.floor(Math.random() * 4)
        ],
        prioridad_proyecto: ["Alta", "Media", "Baja", "Crítica"][
          Math.floor(Math.random() * 4)
        ],
  ...
  const generateRandomFinancialData = (count: number) => {
    return Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        tipo_transaccion: ["Gasto", "Ingreso", "Inversión"][
          Math.floor(Math.random() * 3)
        ],
        categoria_gasto: [
          "Software",
        ...
        importe: importe,
        divisa: "EUR",
        departamento_gasto: ["IT", "Marketing", "Ventas", "RRHH", "Finanzas"][
          Math.floor(Math.random() * 5)
        ],
        ...
        metodo_pago_fin: ["Transferencia", "Tarjeta", "Cheque", "Efectivo"][
          Math.floor(Math.random() * 4)
        ],
        ...
        proyecto_asociado:
          Math.random() > 0.5
            ? `Proyecto ${Math.floor(Math.random() * 50) + 1}`
            : "",
        observaciones:
          Math.random() > 0.7
