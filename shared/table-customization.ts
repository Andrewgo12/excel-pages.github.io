...
export interface ColumnCustomization {
  ...
  formatter?: 'default' | 'currency' | 'percentage' | 'date' | 'datetime' | 'number' | 'custom';
...
export interface TableCustomizationPreset {
  ...
  name: string;
  description: string;
  category: 'business' | 'creative' | 'minimal' | 'colorful' | 'classic' | 'modern' | 'custom';
...
export const BUILT_IN_PRESETS: TableCustomizationPreset[] = [
  {
    ...
    customization: {
      headerFont: { ...DEFAULT_HEADER_FONT_SETTINGS, family: 'system-ui, sans-serif' },
  ...
  },
  {
    ...
    customization: {
      headerFont: { ...DEFAULT_HEADER_FONT_SETTINGS, weight: 'normal', size: 13 },
  ...
  },
  {
    ...
    customization: {
      headerFont: { ...DEFAULT_HEADER_FONT_SETTINGS, family: 'monospace', size: 13 },
...
export const generateTableStyles = (customization: TableCustomization) => {
  const { colorScheme, headerColorScheme, cellFont, headerFont, borderSettings, spacing } = customization;
